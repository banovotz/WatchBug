...
import { db } from './Db';
let itemsRef = db.ref('/watches').orderByChild("brand").limitToFirst(10).equalTo("Seiko");
...
export default class Watches extends Component {
  ...
  _renderItem = ({ item }) => (
    ...
    <AddItem></AddItem>
            <Button
                title={item.name}
                onPress={() => this.props.navigation.navigate('Details', {title: item.name, image: item.image} )}
              >
              </Button>
